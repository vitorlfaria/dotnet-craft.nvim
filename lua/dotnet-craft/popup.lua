...
MaxHeight = math.floor(vim.o.lines * 0.6)
Width = 60
Line = 5
...
function popup.create(what, vim_options)
	...
	if type(what) == "number" then
		...
		if vim_options.padding then
			...
			for index = 1, #what do
                local item = what[index]
                if type(item) == "table" then
                    local indent = string.rep(" ", item.level)
                    table.insert(lines, string.format("%s%s%s", left_padding, indent .. item.icon .. item.name, right_padding))
                else
                    table.insert(lines, string.format("%s%s%s", left_padding, item, right_padding))
                end
			end
